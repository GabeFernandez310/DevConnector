...
router.delete("/", auth, async (req, res) => {
  try {
    //remove profile and user from DB
...
router.put(
  ...
  async (req, res) => {
    ...
    //Destructure data (pull out data from the request body)
    const {
      title,
      company,
      location,
    ...
    //Create an object with the destructured data
    const newExp = {
      title,
      company,
      location,
...
router.delete("/experience/:exp_id", auth, async (req, res) => {
  try {
    //find profile according to information passed in token
    const profile = await Profile.findOne({ user: req.user.id });
    //remove experience corresponding with passed in id
    //    Crete array of item ids, then check the index of the matching one from the returned array
